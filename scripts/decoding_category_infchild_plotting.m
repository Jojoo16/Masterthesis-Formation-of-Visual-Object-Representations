...
%% Decoding in time in infants & children - Plotting Time Course for Category Decoding
% -------------------------------------------------------------------
...
input_folder_i  = fullfile(projectRoot, 'results', 'DA_mean', 'Infants_100', 'cat');
input_folder_c  = fullfile(projectRoot, 'results', 'DA_mean', 'Children_100', 'cat');
...
res_inf = load(fullfile(res_infant, 'results_cat.mat'));
sig_inf = res_inf.results.pvals < 0.05;
peak_inf = res_inf.results.maxLat;
start_inf = res_inf.perm_results.ClusterStart;
end_inf = res_inf.perm_results.ClusterEnd;
...
res_chi = load(fullfile(res_child, 'results_cat.mat'));
sig_chi = res_chi.results.pvals < 0.05;
peak_chi = res_chi.results.maxLat;
start_chi = res_chi.perm_results.ClusterStart;
end_chi = res_chi.perm_results.ClusterEnd;
...
%     title('Time course of decoding Category');
...
savefig(figure(1), fullfile(output_folder, 'timcourse_cat_infchild_sem.fig'));
saveas(figure(1), fullfile(output_folder, 'timecourse_cat_infchild_sem.png'));
